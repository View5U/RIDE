function [tau, currentResult] = TuneThreshold( output, target, bAllOne, metricIndex) %#ok<*INUSD,*NASGU,*DEFNU>
% Tune the threshold for multi-label learning algorithms on the training data with one evaluation metric
...
for t = 1:length(tau_range)
    ...
    if bAllOne == 1 % set to only one threshold for all the class labels
        ...
        % tempResult = evaluateHL(target, predict_target);
        ...
        for l = 1:num_class
            ...
            % tempResult = 1 - evaluateF1(target(l,:), predict_target_l);
